...
include "connect.php";
session_start();
$tgl_checkin = $_SESSION['checkin'];
$tgl_checkout = $_SESSION['checkout'];
...
WHERE no_kamar not in (select no_kamar
                       from transaksi_sewa
                       where status != 1 and
                        id_transaksi in (select id_transaksi
                                              from transaksi_sewa
...
<br><br>
 ...
 <center>     
<form method="post" action="phppesanflamboyan2.php">
  <p>Nomor Kamar</p>
        <div >
     <?php
      $d=0;$a=0;
